...
export class WorkflowEngine implements IWorkflowEngine {
	...
	constructor(
		...
		readonly _registry: AgentRegistry,
	...
	async stopSupervisors(taskId: string): Promise<void> {
		try {
			...
			await (this.steeringManager as any).stopSupervisors?.(taskId);
		} catch (err) {
			const message = err instanceof Error ? err.message : String(err);
	...
	async executeSideEffects(effects: SideEffect[]): Promise<void> {
		...
		for (const spawn of spawns) {
			try {
				const result = await this.dispatchAgent(spawn.agentRole, { id: spawn.taskId } as any, {
